...
import { fetchFeaturedCuration, saveFeaturedCuration } from "@/services/admin/homepage.service";
...
export function FeaturedEventsManager() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ["admin-featured-curation"],
    queryFn: fetchFeaturedCuration,
  });

  const { data: liveEventsRes } = useQuery({
    queryKey: ["admin-events-live-picker"],
  ...
  const saveMut = useMutation({
    mutationFn: () => saveFeaturedCuration(selected.map((e) => e._id)),
    onSuccess: async () => {
      toast.success("Featured events saved");
      setDirty(false);
      await qc.invalidateQueries({ queryKey: ["admin-featured-curation"] });
    },
  ...
  return (
    <div>
      <AdminPageHeader
        ...
        description="Choose and order events for the homepage Featured section. Drag to reorder."
      />
      ...
      <div className="mb-6 flex flex-wrap items-center justify-between gap-3">
        ...
        >
          {saveMut.isPending ? "Saving…" : "Save featured order"}
        </Button>
      </div>
          ...
          <div>
            ...
            {selected.length === 0 ? (
              <p className="rounded-xl border border-dashed border-white/10 p-8 text-center text-sm text-zinc-500">
                No featured events yet. Add live events from the panel on the right.
              </p>
            ) : (
